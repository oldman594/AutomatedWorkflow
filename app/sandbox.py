...
class DockerShellExecutor:
    ...
    def run(self, command: str, cwd: Path, timeout: int, task_id: str | None) -> CommandResult:
        if shutil.which("docker") is None:
            raise RepositoryError(
                "Docker sandbox is required but the docker CLI is unavailable. "
                "Install Docker or explicitly set AUTOFLOW_SANDBOX_MODE=host for trusted development."
            )
    ...
    def _docker_command(self, container_name: str, cwd: Path, command: str) -> list[str]:
        return [
            ...
            "HOME=/tmp/home",
            "--mount",
            f"type=bind,src={cwd.resolve()},dst=/workspace,rw",
            "--workdir",
            "/workspace",
            self.config.image,
            "/bin/bash",
